...
| **`terraform plan -out tfplan`** | <br> **`terraform plan`** It calculates what will be created, changed, or destroyed. </br> **`-out tfplan`** Save this plan in a file called **`tfplan`**. Then, apply exactly that plan with **`terraform apply tfplan`**. |
